...
object BindingSeqT:
  sealed trait Patch[A]:
    def withOffset(offset: Int): Patch[A]
    def sizeIncremental: Int
  ...
  object Patch:
    ...
    final case class Splice[A](index: Int, deleteCount: Int, newItems: Iterable[A]) extends Patch[A]:
      def withOffset(offset: Int) = copy(index = index + offset)
      def sizeIncremental = newItems.size - deleteCount
        ...
        def handleUpstreamEvent(upstreamEvent: EventStep[A]): EventStep[B] =
          upstreamEvent match
            case Yield(Patch.Splice(patchIndex, numberOfSlicesDeleted, newItems), s) =>
              ...
              val Measure(`patchIndex`, mappedIndex, _) = left.measure.getOrElse(Monoid[Measure].zero)
              val Measure(`numberOfSlicesDeleted`, numberOfItemsDeleted, _) =
                deleted.measure.getOrElse(Monoid[Measure].zero)
