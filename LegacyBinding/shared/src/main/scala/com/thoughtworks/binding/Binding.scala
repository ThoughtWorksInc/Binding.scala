...
object LegacyBinding extends LegacyJSBinding:

  inline def apply[A](
      inline a: A
  )(using Monad[DefaultFuture]): LegacyBinding[A] =
    lazy val tail = {
      ...
      CovariantStreamT.memoize(
        CovariantStreamT
          ...
          }.*[LegacyBinding](a).distinctUntilChanged)
      )
    }
    CovariantStreamT(
      StreamT(
  ...
  extension [A](binding: LegacyBinding[A])
    @tailrec
    ...
    def dropHistory(using Applicative[DefaultFuture]): LegacyBinding[A] =
      CovariantStreamT(
        StreamT(
          Applicative[DefaultFuture].point(
            Skip(new (() => StreamT[DefaultFuture, A]) {
              @volatile
              private var cache: LegacyBinding[A] = binding
              def apply() =
    ...
    @tailrec
    private def dropCompletedStrict: LegacyBinding[A] =
      binding.step.value match
        case None | Some(Success(Done())) | Some(Failure(_)) =>
          binding
    ...
    def dropCompleted(using Applicative[DefaultFuture]): LegacyBinding[A] =
      CovariantStreamT(
        StreamT(
          Applicative[DefaultFuture].point(
            Skip(new (() => StreamT[DefaultFuture, A]) {
  ...
  object BindingSeq:
    ...
    ): LegacyBinding[(Snapshot[A], Patch[A])] = CovariantStreamT apply StreamT(
    ...
    @inline def fromMutationStream[A](
      ...
      object Cache:
        ...
        def dropHistoryAndUpdateCache(): LegacyBinding[Patch[A]] = {
  ...
  opaque type Constant[+A] <: LegacyBinding[A] = LegacyBinding[A]
  object Constant:
    def apply[A](a: A)(using Applicative[DefaultFuture]): LegacyBinding[A] =
      CovariantStreamT.pure(a)

  def empty[A](using Applicative[DefaultFuture]): LegacyBinding[A] =
  ...
  private[binding] final class Pipe[A]
    ...
    def read: LegacyBinding[A] = CovariantStreamT(StreamT(promise.future))
    def writeHead(a: A): Unit =
      promise.success(StreamT.Yield(a, Pipe()))
    def writeTail(using ExecutionContext): LegacyBinding[A => Unit] =
    ...
    def write(using ExecutionContext): LegacyBinding[A => Unit] =
      writeHead :: writeTail

  @inline def pipe[A](using
      ExecutionContext
  ): (LegacyBinding[A], LegacyBinding[A => Unit]) =
    val p = Pipe[A]()
    (p.read, p.write)
  ...
  opaque type Var[A] <: LegacyBinding[A] = LegacyBinding[A]
...
type LegacyBinding[+A] = CovariantStreamT[DefaultFuture, A]
