...
object BindingTest extends TestSuite {
  ...
  def tests = TestSuite {
    ...
    'ForYieldWithFilter {
      ...
      assert(mapped.patchedPublisher.subscribers.length == 1)
      assert(mapped.changedPublisher.subscribers.length == 1)
      assert(source.patchedPublisher.subscribers.length > 0)
      assert(source.changedPublisher.subscribers.length > 0)
      source.addChangedListener(sourceEvents.listener)
      source.addPatchedListener(sourceEvents.listener)
      assert(mapped.patchedPublisher.subscribers.length == 1)
      assert(mapped.changedPublisher.subscribers.length == 1)
      assert(source.patchedPublisher.subscribers.length > 1)
      assert(source.changedPublisher.subscribers.length > 1)
    ...
    'ForYield {
      ...
      assert(mapped.patchedPublisher.subscribers.length == 1)
      assert(mapped.changedPublisher.subscribers.length == 1)
      assert(source.patchedPublisher.subscribers.nonEmpty)
      assert(source.changedPublisher.subscribers.nonEmpty)
      source.addChangedListener(sourceEvents.listener)
      source.addPatchedListener(sourceEvents.listener)
      assert(mapped.patchedPublisher.subscribers.length == 1)
      assert(mapped.changedPublisher.subscribers.length == 1)
