...
@dom
def tableRow(contact: Contact): Binding[TableRow] = {
  <tr>
    <td>
      { contact.name.bind }
    </td>
    ...
    <td>
      <button onclick=
        { event: Event => contact.name := "Modified Name" }
      >
        Modify the name
      </button>
    </td>
...
def table: Binding[BindingSeq[Node]] = {
  <div>
    <button onclick=
      { event: Event => data.get += Contact(Var("Yang Bo"), Var("yang.bo@rea-group.com")) }
    >
      Add a contact
    </button>
  </div>
  <table border="1" cellPadding="5">
    ...
    <tbody>
      { for (contact <- data) yield tableRow(contact).bind }
    </tbody>
  </table>
}
```
